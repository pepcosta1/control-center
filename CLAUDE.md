...
- **Color**: un color dominant + un accent nítid, com a variables CSS. Res de gradients morats sobre blanc. **La paleta canònica del projecte (tema "casa de nit": verd tinta `--bg` + ambre `--accent`, variables glass, tipografia) està documentada a `ARCHITECTURE.md` §8 — fes servir sempre aquestes variables, no inventis colors nous.**
...
- **TV Samsung (SmartThings)**: OAuth amb refresc automàtic de tokens (app API_ONLY "centre-de-control-tv" creada amb la CLI de SmartThings el 10 jul 2026). Els PAT nous de Samsung caduquen a les 24h — per això es va abandonar el PAT (els dos que hi havia van morir). Variables: SMARTTHINGS_CLIENT_ID/SECRET/REDIRECT_URI + SMARTTHINGS_TV_DEVICE_ID; tokens a data/store.json (renovació 5 min abans de caducar, retry a 401, keepalive cada 12h perquè el refresh token rotatiu no caduqui als 30 dies). Autorització inicial: botó «Connecta amb SmartThings» a la targeta TV (/api/smartthings/login), un sol cop. Si mai cal recrear l'app: PAT temporal amb w:apps + `npx @smartthings/cli apps:create --input <json>`.
